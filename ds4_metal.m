...
static int g_skip_model_residency;
...
#define DS4_METAL_MAX_MODEL_VIEWS 48
...
#define DS4_EXPERT_DMA_SLOTS 6
static id<MTLBuffer> g_expert_dma[DS4_EXPERT_DMA_SLOTS];
static void *g_expert_dma_ptrs[DS4_EXPERT_DMA_SLOTS];
static uint64_t g_expert_dma_size;

static id<MTLBuffer> g_batch_stage_gate;
static id<MTLBuffer> g_batch_stage_up;
static id<MTLBuffer> g_batch_stage_down;
static void *g_batch_stage_gate_ptr;
static void *g_batch_stage_up_ptr;
static void *g_batch_stage_down_ptr;
...
static int ds4_gpu_model_residency_request_views(void) {
    if (g_model_view_count == 0 || g_skip_model_residency || getenv("DS4_METAL_NO_RESIDENCY") != NULL) return 1;
...
static int ds4_gpu_map_model_views(
    ...
    const int request_residency = !g_skip_model_residency &&
    ...
    const int warm_model_views = !g_skip_model_residency &&
                                 getenv("DS4_METAL_NO_RESIDENCY") == NULL &&
...
static int ds4_gpu_create_views_for_span(
        const void *model_map,
        uint64_t    model_size,
        uint64_t    map_offset,
        uint64_t    map_size) {
    ...
    const uint64_t overlap = round_up_u64(DS4_METAL_MODEL_MAX_TENSOR_BYTES, page) + page;
    if (max_buffer == 0 || max_buffer <= overlap) return 0;

    const uint64_t step = max_buffer - overlap;
    uint64_t off = 0;
    while (off < mapped_model_size) {
        if (g_model_view_count == DS4_METAL_MAX_MODEL_VIEWS) {
            fprintf(stderr, "ds4: too many model views in span mapping\n");
            return 0;
        }
        uint64_t view_bytes = mapped_model_size - off;
        if (view_bytes > max_buffer) view_bytes = max_buffer;
        ...
        id<MTLBuffer> buffer = [g_device newBufferWithBytesNoCopy:(void *)(model_addr + page_model_offset + off)
                                                      ...
                                                      deallocator:nil];
        if (!buffer) {
            fprintf(stderr, "ds4: Metal span view failed at %.2f GiB, size %.2f GiB\n",
                    (double)off / (1024.0 * 1024.0 * 1024.0),
                    (double)view_bytes / (1024.0 * 1024.0 * 1024.0));
            return 0;
        }
...
int ds4_gpu_set_model_map_spans(const void *model_map, uint64_t model_size,
    ...
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!model_map || model_size == 0 || n_spans <= 0) return 0;

    @autoreleasepool {
        ds4_gpu_model_residency_clear();
        g_model_map_ptr = model_map;
        g_model_map_size = model_size;
        g_model_mapped_offset = spans[0].offset;
        uint64_t last_end = spans[0].offset + spans[0].size;
        for (int i = 1; i < n_spans; i++) {
            uint64_t end = spans[i].offset + spans[i].size;
            if (end > last_end) last_end = end;
        }
        g_model_mapped_size = last_end - g_model_mapped_offset;

        const double t0 = ds4_gpu_now_ms();
        uint64_t total_mapped = 0;
        for (int i = 0; i < n_spans; i++) {
            if (!ds4_gpu_create_views_for_span(model_map, model_size,
                                               spans[i].offset, spans[i].size)) {
                ds4_gpu_model_residency_clear();
                return 0;
            }
            total_mapped += spans[i].size;
        }
        const double t_mapped = ds4_gpu_now_ms();

        const int request_residency = !g_skip_model_residency &&
                                      getenv("DS4_METAL_NO_RESIDENCY") == NULL;
        if (request_residency) ds4_gpu_progress_begin("requesting Metal residency");
        if (!ds4_gpu_model_residency_request_views()) {
            if (request_residency) ds4_gpu_progress_failed();
            return 0;
        }
        ...
        fprintf(stderr,
                "ds4: Metal mapped %d non-expert spans as %u views in %.3f ms, "
                "residency %.3f ms, warmup %.3f ms (%.2f MiB from %d spans)\n",
                n_spans, g_model_view_count,
                t_mapped - t0,
                t_resident - t_mapped,
                t_warm - t_warm0,
                total_mapped / 1024.0 / 1024.0,
                n_spans);
        if (!warmed) return 0;
        return 1;
    }
...
void ds4_gpu_set_skip_model_residency(int skip) {
    g_skip_model_residency = skip;
}

int ds4_gpu_init_expert_streaming(uint64_t expert_stride) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    /* 2MB alignment for NVMe DMA throughput (3.6x faster per Flash-iOS benchmarks) */
    const size_t alignment = 2u * 1024u * 1024u;
    uint64_t alloc_size = (expert_stride + alignment - 1) / alignment * alignment;
    g_expert_dma_size = alloc_size;
    for (int i = 0; i < DS4_EXPERT_DMA_SLOTS; i++) {
        void *ptr = NULL;
        if (posix_memalign(&ptr, alignment, (size_t)alloc_size) != 0 || !ptr) {
            fprintf(stderr, "ds4: failed to allocate 2MB-aligned expert DMA buffer\n");
            return 0;
        }
        ...
        if (!g_expert_dma[i]) {
            ...
            free(ptr);
            g_expert_dma_ptrs[i] = NULL;
            return 0;
        }
        g_expert_dma[i].label = [NSString stringWithFormat:@"ds4_expert_dma_%d", i];
    }
    fprintf(stderr, "ds4: allocated %d expert DMA buffers, %.2f MB each (2MB aligned)\n",
            DS4_EXPERT_DMA_SLOTS, (double)alloc_size / (1024.0 * 1024.0));
    return 1;
}

void *ds4_gpu_expert_dma_ptr(int slot) {
    if (slot < 0 || slot >= DS4_EXPERT_DMA_SLOTS) return NULL;
    return g_expert_dma_ptrs[slot];
}

int ds4_gpu_expert_dma_slot_count(void) {
    return DS4_EXPERT_DMA_SLOTS;
}

int ds4_gpu_init_batch_staging(uint64_t gate_expert_bytes,
    ...
    const MTLResourceOptions opts = MTLResourceStorageModeShared |
                                    MTLResourceCPUCacheModeWriteCombined |
                                    MTLResourceHazardTrackingModeUntracked;
    const uint64_t gate_total = 256ull * gate_expert_bytes;
    const uint64_t up_total   = 256ull * up_expert_bytes;
    const uint64_t down_total = 256ull * down_expert_bytes;

    g_batch_stage_gate = [g_device newBufferWithLength:(NSUInteger)gate_total options:opts];
    g_batch_stage_up   = [g_device newBufferWithLength:(NSUInteger)up_total options:opts];
    g_batch_stage_down = [g_device newBufferWithLength:(NSUInteger)down_total options:opts];
    if (!g_batch_stage_gate || !g_batch_stage_up || !g_batch_stage_down) {
        g_batch_stage_gate = nil;
        g_batch_stage_up = nil;
        g_batch_stage_down = nil;
        return 0;
    }
    g_batch_stage_gate.label = @"ds4_batch_stage_gate";
    g_batch_stage_up.label   = @"ds4_batch_stage_up";
    g_batch_stage_down.label = @"ds4_batch_stage_down";
    g_batch_stage_gate_ptr = [g_batch_stage_gate contents];
    g_batch_stage_up_ptr   = [g_batch_stage_up contents];
    g_batch_stage_down_ptr = [g_batch_stage_down contents];
    fprintf(stderr, "ds4: batch staging buffers allocated: gate %.1f + up %.1f + down %.1f = %.1f MiB\n",
            (double)gate_total / (1024.0 * 1024.0),
            (double)up_total / (1024.0 * 1024.0),
            (double)down_total / (1024.0 * 1024.0),
            (double)(gate_total + up_total + down_total) / (1024.0 * 1024.0));
    return 1;
}

void *ds4_gpu_batch_stage_gate_ptr(void) { return g_batch_stage_gate_ptr; }
void *ds4_gpu_batch_stage_up_ptr(void)   { return g_batch_stage_up_ptr; }
void *ds4_gpu_batch_stage_down_ptr(void) { return g_batch_stage_down_ptr; }
...
// Staging buffers for expert streaming (allocated on first use)
static id<MTLBuffer> g_stream_gate_combined;
static id<MTLBuffer> g_stream_up_combined;
static id<MTLBuffer> g_stream_down_combined;
static id<MTLBuffer> g_stream_selected_remap;
...
int ds4_gpu_routed_moe_one_streamed(
    ...
    if (!out || !gate || !up || !mid || !x || !selected || !weights ||
        n_expert == 0 || n_expert > DS4_EXPERT_DMA_SLOTS) {
        return 0;
    }
    if ((expert_in_dim % 256u) != 0 || (expert_mid_dim % 256u) != 0) return 0;
    ...
    @autoreleasepool {
        ...
        const uint64_t x_bytes_needed = (uint64_t)expert_in_dim * sizeof(float);
        const uint64_t mid_bytes_needed = (uint64_t)n_expert * expert_mid_dim * sizeof(float);
        const uint64_t out_bytes_needed = (uint64_t)out_dim * sizeof(float);
        if (!xbuf || !gatebuf || !upbuf || !midbuf || !outbuf || !weightsbuf ||
            ds4_gpu_tensor_bytes(x) < x_bytes_needed ||
            ds4_gpu_tensor_bytes(gate) < mid_bytes_needed ||
            ds4_gpu_tensor_bytes(up) < mid_bytes_needed ||
            ds4_gpu_tensor_bytes(mid) < mid_bytes_needed ||
            ds4_gpu_tensor_bytes(out) < out_bytes_needed ||
            ds4_gpu_tensor_bytes(weights) < (uint64_t)n_expert * sizeof(float)) {
            fprintf(stderr, "ds4: Metal streamed MoE received undersized buffers\n");
            return 0;
        }
        if (n_expert > 1 &&
            (!expertsbuf ||
             ds4_gpu_tensor_bytes(experts) < (uint64_t)n_expert * out_dim * sizeof(float))) {
            fprintf(stderr, "ds4: Metal streamed MoE received undersized expert output buffer\n");
            return 0;
        }
        ...
        id<MTLComputePipelineState> down_mv_pipeline = ds4_gpu_routed_mv_pipeline(down_type);
        if (gate_nr0 == 0 || down_nr0 == 0 || !gate_mv_pipeline || !down_mv_pipeline) {
            fprintf(stderr, "ds4: unsupported streamed MoE quant types gate=%u down=%u\n",
                    gate_type, down_type);
            return 0;
        }

        // Key difference: ne02 = n_expert (6) instead of 256
        ds4_gpu_mul_mv_id_args gate_args =
            ds4_gpu_make_mul_mv_id_args(expert_in_dim, expert_mid_dim, n_expert,
                                          gate_row_bytes, gate_expert_bytes,
                                          1, n_expert, n_tokens, gate_nr0);
        ds4_gpu_mul_mv_id_args down_args =
            ds4_gpu_make_mul_mv_id_args(expert_mid_dim, out_dim, n_expert,
                                          down_row_bytes, down_expert_bytes,
                                          n_expert, n_expert, n_tokens, down_nr0);
        ...
        if (fuse_pair_swiglu) {
            ...
            ok = ds4_gpu_encode_mul_mv_id_pair_swiglu(cb,
                                                        ...
                                                        &gate_args,
                                                        &act_args,
                                                        g_stream_gate_combined,
                                                        0,
                                                        g_stream_up_combined,
                                                        0,
                                                        xbuf,
                                                        ds4_gpu_tensor_offset(x),
                                                        gatebuf,
        ...
        if (!ds4_gpu_finish_command_buffer(cb, owned, "streamed MoE")) return 0;
    }
...
int ds4_gpu_routed_moe_one_streamed_pair_swiglu(
        ...
        const ds4_gpu_tensor *x) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!gate || !up || !mid || !x || !selected || !weights ||
        n_expert == 0 || n_expert > DS4_EXPERT_DMA_SLOTS) return 0;
    if ((expert_in_dim % 256u) != 0 || (expert_mid_dim % 256u) != 0) return 0;
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> midbuf = ds4_gpu_tensor_buffer(mid);
        id<MTLBuffer> weightsbuf = ds4_gpu_tensor_buffer(weights);
        if (!xbuf || !gatebuf || !upbuf || !midbuf || !weightsbuf) return 0;

        const uint32_t n_tokens = 1;
        const uint32_t pair_rows = n_tokens * n_expert;
        const uint32_t gate_nr0 = ds4_gpu_routed_mv_nr0(gate_type);
        if (gate_nr0 == 0) return 0;
        ...
        int ok = 1;
        if (pair_swiglu_pipeline && !g_quality_mode &&
            getenv("DS4_METAL_MOE_WRITE_CLAMPED_ACT") == NULL &&
            getenv("DS4_METAL_DISABLE_ROUTED_PAIR_SWIGLU_FUSION") == NULL) {
            ds4_gpu_dsv4_moe_swiglu_weight_args act_args = {
                .width = expert_mid_dim,
            ...
            ok = ds4_gpu_encode_mul_mv_id_pair_swiglu(cb,
                pair_swiglu_pipeline, &gate_args, &act_args,
                g_stream_gate_combined, 0,
                g_stream_up_combined, 0,
                ...
                g_stream_selected_remap, 0,
                weightsbuf, ds4_gpu_tensor_offset(weights),
                gate_smem, 2, false);
        } else {
            id<MTLComputePipelineState> gate_mv_pipeline = ds4_gpu_routed_mv_pipeline(gate_type);
            if (!gate_mv_pipeline) return 0;
            ok = ds4_gpu_encode_mul_mv_id(cb, gate_mv_pipeline, &gate_args,
                g_stream_gate_combined, 0,
                xbuf, ds4_gpu_tensor_offset(x),
                gatebuf, ds4_gpu_tensor_offset(gate),
                g_stream_selected_remap, 0, gate_smem, 2, false) &&
                 ds4_gpu_encode_mul_mv_id(cb, gate_mv_pipeline, &gate_args,
                g_stream_up_combined, 0,
                xbuf, ds4_gpu_tensor_offset(x),
                upbuf, ds4_gpu_tensor_offset(up),
                g_stream_selected_remap, 0, gate_smem, 2, false);
        ...
        if (!ok) return 0;
        if (!ds4_gpu_finish_command_buffer(cb, owned, "streamed MoE pair+swiglu")) return 0;
    }
    return 1;
}
...
int ds4_gpu_routed_moe_one_streamed_down(
        ...
        uint32_t                n_expert) {
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!out || !mid || n_expert == 0 || n_expert > DS4_EXPERT_DMA_SLOTS) return 0;

    @autoreleasepool {
        id<MTLBuffer> midbuf = ds4_gpu_tensor_buffer(mid);
        id<MTLBuffer> outbuf = ds4_gpu_tensor_buffer(out);
        id<MTLBuffer> expertsbuf = experts ? ds4_gpu_tensor_buffer(experts) : nil;
        if (!midbuf || !outbuf) return 0;

        const uint32_t n_tokens = 1;
        const uint32_t down_nr0 = ds4_gpu_routed_mv_nr0(down_type);
        id<MTLComputePipelineState> down_mv_pipeline = ds4_gpu_routed_mv_pipeline(down_type);
        if (down_nr0 == 0 || !down_mv_pipeline) return 0;
        ...
        const bool direct_down_sum = !g_quality_mode && n_expert == 6 &&
                                      n_tokens == 1 && down_sum6_pipeline != nil;

        if (direct_down_sum) {
            ok = ds4_gpu_encode_mul_mv_id_sum6(cb, down_sum6_pipeline, &down_args,
                g_stream_down_combined, 0,
                midbuf, ds4_gpu_tensor_offset(mid),
                outbuf, ds4_gpu_tensor_offset(out),
                g_stream_selected_remap, 0, down_smem, 2);
        } else {
            id<MTLBuffer> down_dst = n_expert == 1 ? outbuf :
                (expertsbuf ? expertsbuf : g_moe_down_scratch_buffer);
            NSUInteger down_dst_off = n_expert == 1 ? ds4_gpu_tensor_offset(out) :
                (expertsbuf ? ds4_gpu_tensor_offset(experts) : 0);
            ok = ds4_gpu_encode_mul_mv_id(cb, down_mv_pipeline, &down_args,
                ...
                g_stream_selected_remap, 0, down_smem, 2, false);
            if (ok && n_expert > 1) {
                ok = ds4_gpu_encode_moe_sum_experts(cb,
                    down_dst, down_dst_off,
                    outbuf, ds4_gpu_tensor_offset(out),
        ...
        if (!ok) return 0;
        if (!ds4_gpu_finish_command_buffer(cb, owned, "streamed MoE down")) return 0;
    }
    return 1;
}
...
int ds4_gpu_routed_moe_batch_staged_tensor(
        ds4_gpu_tensor       *out,
        ds4_gpu_tensor       *gate,
        ds4_gpu_tensor       *up,
        ds4_gpu_tensor       *mid,
    ...
    if (!g_initialized && !ds4_gpu_init()) return 0;
    if (!out || !gate || !up || !mid || !x || !selected || !weights ||
        !g_batch_stage_gate || !g_batch_stage_up || !g_batch_stage_down ||
        n_tokens == 0 || n_expert == 0 || n_expert > 6) {
        return 0;
    }
    if ((expert_in_dim % 256u) != 0 || (expert_mid_dim % 256u) != 0) return 0;
    ...
    @autoreleasepool {
        ...
        id<MTLBuffer> gate_buf = g_batch_stage_gate;
        id<MTLBuffer> up_buf = g_batch_stage_up;
        id<MTLBuffer> down_buf = g_batch_stage_down;
        ...
        if (down_type == DS4_METAL_TENSOR_Q2_K)
            ...
            down_sum6_pipeline = g_moe_mul_mv_id_q4_k_sum6_pipeline;
        const bool direct_down_sum =
            !g_quality_mode && !use_mm_id && n_expert == 6 &&
            n_tokens <= 4u && down_sum6_pipeline != nil;
        ...
        if (use_mm_id) {
            ok = ds4_gpu_encode_mul_mm_id_map(cb, map_pipeline, &gate_map_args,
                                                ...
                                                ds4_gpu_tensor_offset(selected));
            if (ok)
                ok = ds4_gpu_encode_mul_mm_id_mapped(cb, gate_mm_pipeline, &gate_mm_args,
                    gate_buf, (NSUInteger)gate_inner, xbuf, ds4_gpu_tensor_offset(x),
                    gatebuf, ds4_gpu_tensor_offset(gate));
            if (ok)
                ok = ds4_gpu_encode_mul_mm_id_mapped(cb, gate_mm_pipeline, &gate_mm_args,
                    up_buf, (NSUInteger)up_inner, xbuf, ds4_gpu_tensor_offset(x),
                    upbuf, ds4_gpu_tensor_offset(up));
        } else {
            ok = ds4_gpu_encode_mul_mv_id(cb, gate_mv_pipeline, &gate_args,
                    gate_buf, (NSUInteger)gate_inner, xbuf, ds4_gpu_tensor_offset(x),
                    gatebuf, ds4_gpu_tensor_offset(gate), selectedbuf,
                    ds4_gpu_tensor_offset(selected), gate_smem, 2, false) &&
                 ds4_gpu_encode_mul_mv_id(cb, gate_mv_pipeline, &gate_args,
                    up_buf, (NSUInteger)up_inner, xbuf, ds4_gpu_tensor_offset(x),
                    upbuf, ds4_gpu_tensor_offset(up), selectedbuf,
                    ds4_gpu_tensor_offset(selected), gate_smem, 2, false);
        }

        const bool use_fused_activation = !g_quality_mode;
        const bool use_mid_f16 = use_mm_id && use_fused_activation && request_mid_f16;
        if (mid_is_f16) *mid_is_f16 = use_mid_f16;
        if (ok && use_fused_activation) {
            ok = ds4_gpu_encode_moe_swiglu_weight(cb,
                gatebuf, ds4_gpu_tensor_offset(gate),
                ...
                midbuf, ds4_gpu_tensor_offset(mid),
                weightsbuf, ds4_gpu_tensor_offset(weights),
                expert_mid_dim, pair_rows, clamp, use_mid_f16);
        } else if (ok) {
            ok = ds4_gpu_encode_swiglu_flat(cb,
                gatebuf, ds4_gpu_tensor_offset(gate),
                upbuf, ds4_gpu_tensor_offset(up),
        ...
        if (ok) {
            if (direct_down_sum) {
                ok = ds4_gpu_encode_mul_mv_id_sum6(cb, down_sum6_pipeline, &down_args,
                    ...
                    outbuf, ds4_gpu_tensor_offset(out), selectedbuf,
                    ds4_gpu_tensor_offset(selected), down_smem, 2);
            } else if (use_mm_id) {
                ok = ds4_gpu_encode_mul_mm_id_mapped(cb, down_mm_pipeline, &down_mm_args,
                    down_buf, (NSUInteger)down_inner, midbuf, ds4_gpu_tensor_offset(mid),
                    down_dst, down_dst_off);
            } else {
                ok = ds4_gpu_encode_mul_mv_id(cb, down_mv_pipeline, &down_args,
                    down_buf, (NSUInteger)down_inner, midbuf, ds4_gpu_tensor_offset(mid),
                    down_dst, down_dst_off, selectedbuf, ds4_gpu_tensor_offset(selected),
                    down_smem, 2, false);
            }
        }
        if (ok && n_expert > 1 && !direct_down_sum) {
            ok = ds4_gpu_encode_moe_sum_experts(cb, down_dst, down_dst_off,
                outbuf, ds4_gpu_tensor_offset(out), out_dim, n_expert, n_tokens);
        }
        if (!ok) return 0;
        if (!ds4_gpu_finish_command_buffer(cb, owned, "routed batch MoE staged")) return 0;
    }
    return 1;
}
